...
class Test_recommend:
    def test_おすすめの本の情報を取得できること(self, monkeypatch):
    ...
    def test_おすすめの本が取得できなかったら空のリストを返すこと(self, monkeypatch):
        def mock_recommend_book_repository_fetch(_, __):
            return []
    ...
    def test_おすすめの本の情報がNoneのケース(self, monkeypatch):
        def mock_recommend_book_repository_fetch(_, __):
            return {"ml-a": "1234567890123"}
    ...
    def test_書影が取得できない場合に書影にNone返値に設定されること(self, monkeypatch):
        def mock_recommend_book_repository_fetch(_, __):
            return {"ml-a": "1234567890123"}
    ...
    def test_モジュール内で例外が発生した場合は返値は空のリストであること(
        self,
        monkeypatch,
    ):
        def mock_recommend_book_repository_fetch(_, __):
    ...
    def test_複数のおすすめの本の情報を取得できること(self, monkeypatch):
        def mock_recommend_book_repository_fetch(_, __):
...
class Test_created_at:
    def test_おすすめ情報の生成時刻を取得できること(self, monkeypatch):
    ...
    def test_メタデータが存在しない場合はNoneが返ること(self, monkeypatch):
        def mock_recommend_book_repository_fetch_metadata(_):
            return None
    ...
    def test_メタデータにタイムスタンプ情報が存在しない場合はNoneが返ること(self, monkeypatch):
        def mock_recommend_book_repository_fetch_metadata(_):
            return {
                "hoge": "hoge_value",
                "fuga": "fuga_value",
    ...
    def test_メタデータ取得中にエラーが発生した場合はNoneが返ること(self, monkeypatch):
        def mock_recommend_book_repository_fetch_metadata(_):
            raise Exception("dummy exception")
